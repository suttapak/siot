...
import { CreateControl, CreateControlDto } from '../delivery/Control';
...
export const CreateControlComponent: React.FC<Props> = (props: Props) => {
  const { register, handleSubmit, reset } = useForm<CreateControlDto>();

  const queryClient = useQueryClient();

  const { mutate, isLoading } = useMutation(
    async (body: CreateControlDto) => {
      body.layout = props.layout;
      body.widget = { id: props.widgetId };
      return await CreateControl(props.boxId, body);
    },
    {
      onSuccess: () => {
  ...
  return (
    ...
    >
      ...
      <div className='w-full pb-2'>
        ...
        <h3 className='text-xl font-bold text-center'>Control</h3>
      </div>
      <hr />
      {/* body */}
      <div className='pt-4'>
