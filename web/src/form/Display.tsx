...
import { CreateDisplayDto, CreateDisplays } from '../delivery/DIsplay';
...
export const CreateDisplayComponent: React.FC<Props> = (props: Props) => {
  const { register, handleSubmit, reset } = useForm<CreateDisplayDto>();

  const queryClient = useQueryClient();

  const { mutate, isLoading } = useMutation(
    async (body: CreateDisplayDto) => {
      body.layout = props.layout;
      body.widget = { id: props.widgetId };
      return await CreateDisplays(props.boxId, body);
    },
    {
      onSuccess: () => {
  ...
  return (
    ...
    >
      ...
      <div className='w-full pb-2'>
        ...
        <h3 className='text-xl font-bold text-center'>Displays</h3>
      </div>
      <hr />
      {/* body */}
      <div className='pt-4'>
      ...
      <div className='py-4'>
        <button type='submit' className='text-lg font-bold flex items-center px-3 py-1 rounded-lg hover:bg-gray-100 transition-colors'>
          <BsStack /> <span className='pl-2'>Add widget</span>{' '}
          {isLoading && (
